...
function kysiKaupadeAndmed($sorttulp="temperatuur", $otsisona=""){
    ...
    $kask=$yhendus->prepare("SELECT teavet.id, temperatuur, maakonnanimi, kuupaev
       FROM teavet, maakonna
       WHERE teavet.maakonna_id=maakonna.id
...
function lisaKaup($temperatuur, $maakonna_id, $kuupaev){
    ...
    $kask=$yhendus->prepare("INSERT INTO
       teavet (temperatuur, maakonna_id, kuupaev)
       VALUES (?, ?, ?)");
...
function kustutaKaup($teavet_id){
    global $yhendus;
    $kask=$yhendus->prepare("DELETE FROM teavet WHERE id=?");
...
function muudaKaup($teavet_id, $temperatuur, $maakonna_id, $kuupaev){
    global $yhendus;
    $kask=$yhendus->prepare("UPDATE teavet SET temperatuur=?, maakonna_id=?, kuupaev=?
                      WHERE id=?");
